...
p_rot <- ggplot(data = df, aes(x = Conc, y = Sg_0)) + geom_point(size = 2.5) + 
  theme_bw() + 
  ...
  geom_smooth(method = 'loess', formula = y ~ x, size = 0.4, color = 'black')
...
p_tra <- ggplot(data = df, aes(x = Conc, y = Sg_0)) + geom_point(size = 2.5) + 
  geom_point(aes(y = Sg_t), color = '#34B1C9', size = 2.5) +
  theme_bw() + 
  labs(y = '5', x = 'Mensurando') +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.text.x = element_text(color = "black"),
...
#savepdf(list(p_rot, p_tra), name = 'SelecEfectoInterf', h = 4, w = 6)
